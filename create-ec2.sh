...
aws ec2 run-instances --image-id ${AMI_ID} --instance-type t3.micro --instance-market-options "MarketType=spot,SpotOptions={SpotInstanceType=persistent,InstanceInterruptionBehavior=stop}" --tag-specifications "ResourceType=spot-instances-request,Tags=[{Key=Name,Value=${NAME}}]" "ResourceType=instance,Tags=[{Key=Name,Value=${NAME}}]" --security-group-ids sg-0b9f9f51362ec2fb4 &>/dev/null
...
aws route53 change-resource-record-sets --hosted-zone-id Z10462863JH70KFPMLMA0 --change-batch file:///tmp/record.json &>/dev/null
